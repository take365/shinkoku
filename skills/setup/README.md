...
- この README は `git clone` 済みのリポジトリで作業する開発者向け
- `uv tool install git+https://...` の利用者向け導入ではなく、ローカルコードをそのまま使う前提
- まずはリポジトリ直下で `uv sync` し、必要なら仮想環境を有効化してから `python -m shinkoku.cli ...` を使う
...
- clone 済みのリポジトリ直下で作業していること
- ローカル環境で `python -m shinkoku.cli ...` が呼べること
